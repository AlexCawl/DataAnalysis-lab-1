...
def FunkSVD_sgd(R, int num_factors=50, double lrate=0.01,
    ...
    cdef double [:] data = np.array(R.data, dtype=np.float_)
    ...
    cdef double[:,:] U = np.random.normal(init_mean, init_std, (n_users, num_factors)).astype(np.float_)
    cdef double[:,:] V = np.random.normal(init_mean, init_std, (n_items, num_factors)).astype(np.float_)
    ...
    cdef double[:] U_i = np.zeros(num_factors, dtype=np.float_)
    cdef double[:] V_j = np.zeros(num_factors, dtype=np.float_)
