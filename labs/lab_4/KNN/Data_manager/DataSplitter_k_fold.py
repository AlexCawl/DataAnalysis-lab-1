...
class DataSplitter_Warm_k_fold(DataSplitter_k_fold):
    ...
    def _split_data_from_original_dataset(self, save_folder_path):
        ...
        for user_id in range(self.n_users):
            ...
            indices_to_suffle = np.arange(len(user_profile), dtype=np.int_)
        ...
        for fold_index in range(self.n_folds):
            URM_fold_object = self.fold_split[fold_index]["URM"]
            URM_fold_object.row = np.array(URM_fold_object.row, dtype=np.int_)
            URM_fold_object.col = np.array(URM_fold_object.col, dtype=np.int_)
            URM_fold_object.data = np.array(URM_fold_object.data, dtype=np.float_)

            self.fold_split[fold_index]["URM"] = sps.csr_matrix(URM_fold_object)
            self.fold_split[fold_index]["items_in_fold"] = np.arange(0, self.n_items, dtype=np.int_)
...
class DataSplitter_ColdItems_k_fold(DataSplitter_k_fold):
    ...
    def _select_balanced_split(self, URM):
        ...
        indices_to_suffle = np.arange(n_items, dtype=np.int_)
