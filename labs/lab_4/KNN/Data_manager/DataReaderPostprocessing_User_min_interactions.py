...
class DataReaderPostprocessing_User_min_interactions(DataReaderPostprocessing):
    ...
    def _load_from_original_file(self):
        ...
        ICM_filter_mask = np.ones(n_items, dtype=np.bool_)
...
def select_users_with_min_interactions(URM, min_interactions = 5, reshape = False):
    ...
    removed_users = np.arange(0, n_users, dtype=np.int_)[user_to_remove_mask]
    ...
    removed_items = np.arange(0, n_items, dtype=np.int_)[items_to_remove_mask]
