...
class MultifacetedMovieTrailerFeatureReader(DataReader):
    ...
    def _load_from_original_file(self):
        ...
        try:
            ...
            print("MultifacetedMovieTrailerFeatureReader: Unable to find data zip file.")
            print("MultifacetedMovieTrailerFeatureReader: Automatic download not available, please ensure the ZIP data file is in folder {}.".format(self.zip_file_folder))
            print("MultifacetedMovieTrailerFeatureReader: Data can be downloaded here: {}".format(self.DATASET_URL))
        ...
        movielens20M.load_data()

        self.URM_all = movielens20M.get_URM_all()
        n_users, n_items = self.URM_all.shape

        # Remove items with no features
        movielens_items_to_keep = []

        for movielens_item_id, movielens_item_index in movielens20M.item_original_ID_to_index.items():

            if movielens_item_id in self.item_original_ID_to_index:
                movielens_items_to_keep.append(movielens_item_index)

        self.URM_all = self.URM_all[:,movielens_items_to_keep]

        # Remove users with no interactions
        self.URM_all = sps.csr_matrix(self.URM_all)
        users_to_keep = np.ediff1d(self.URM_all.indptr)>0

        self.URM_all = self.URM_all[users_to_keep,:]

        self.user_original_ID_to_index = movielens20M.user_original_ID_to_index.copy()

        self.user_original_ID_to_index = reconcile_mapper_with_removed_tokens(self.user_original_ID_to_index,
                                                                              np.arange(n_users, dtype=np.int_)[np.logical_not(users_to_keep)])
        ...
        for ICM_count, (ICM_name, ICM_path) in enumerate(ICM_name_to_path.items()):
            ...
            print("MultifacetedMovieTrailerFeatureReader: Loading ICM {} of {}: '{}'".format(ICM_count+1, len(ICM_name_to_path), ICM_name))
            ...
            self.__setattr__("tokenToFeatureMapper_" + ICM_name, ICM_mapper_object)
        ...
        print("MultifacetedMovieTrailerFeatureReader: loading complete")
