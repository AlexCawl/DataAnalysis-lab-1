...
class EvaluatorNegativeItemSample(Evaluator):
    ...
    def evaluateRecommender(self, recommender_object):
        ...
        self.__all_items = np.arange(0, self.n_items, dtype=np.int_)
        self.__all_items_mask = np.ones(len(self.__all_items), dtype=np.bool_)
        self.__no_items_mask = np.zeros(len(self.__all_items), dtype=np.bool_)
